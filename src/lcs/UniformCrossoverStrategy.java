...
public class UniformCrossoverStrategy implements CrossoverStrategy {
	...
	@Override
	public ExplanationRule[] crossover(ExplanationRule firstParent, ExplanationRule secondParent) {
		List<Object> firstParentVector = firstParent.norm.getConditions().getVector();
        List<Object> secondParentVector = secondParent.norm.getConditions().getVector();
        List<Object> firstChildVector = new ArrayList<>();
        List<Object> secondChildVector = new ArrayList<>();
        ...
        Explanation explanation = firstParent.norm.consequent;
        ExplanationRule firstChild = new ExplanationRule(new ExplanationNorm(Context.fromVector(firstChildVector), explanation));
        ExplanationRule secondChild = new ExplanationRule(new ExplanationNorm(Context.fromVector(secondChildVector), explanation));
        return new ExplanationRule[]{firstChild, secondChild};
	}
}
