...
public class KovacsDeletionScheme implements DeletionStrategy {
    ...
    public boolean prune(RuleSet ruleSet) {
        ...
        for (Rule rule : rules) {
            double weight = (double) rule.actionSetSize * (double) rule.numerosity;
            if (weight == 0) {
            ...
            if (weight < 0) {//for bug
                Debugger.debug(weight, "weight", rule.actionSetSize, "actionSetSize", rule.numerosity, "numerosity", meanFitness, "meanFitness", rule.fitness, "rule.fitness");
            }
	...
	@Override
	public boolean prune(ExplanationRuleSet ruleSet) {
		int totalNumerosity = 0;
        ...
        ExplanationRule ruleToBeDeleted = randomizer.choice(rules, weights);
        ruleSet.removeRule(ruleToBeDeleted);
        return true;
	}
}
