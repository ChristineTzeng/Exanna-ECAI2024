...
public class DistinctActionsBasedCoveringStrategy implements CoveringStrategy {
	...
	public boolean initiateCovering(ExplanationRuleSet matchSet) {
		return matchSet.getRules().stream()
                .map(rule -> rule.getNorm().consequent)
                .distinct()
                .count() < (Explanation.values().length);
	}

	@Override
	public List<ExplanationRule> getCoveringRules(Context context, ExplanationRuleSet matchSet) {
		Set<Explanation> remainingExplanations = new LinkedHashSet<>(Arrays.asList(Explanation.values()));
        List<ExplanationRule> rules = matchSet.getRules();
        for (ExplanationRule rule : rules) {
            remainingExplanations.remove(rule.getNorm().consequent);
        }
        List<Explanation> remExplanationList = new ArrayList<>(remainingExplanations);
        List<ExplanationRule> coveringRules = new ArrayList<>();
        while (!remainingExplanations.isEmpty()) {
            Context randomContext = getRandomCoveringContext(context);
            Explanation randomExplanation = remExplanationList.get(randomizer.random.nextInt(remExplanationList.size()));
            remainingExplanations.remove(randomExplanation);
            coveringRules.add(new ExplanationRule(new ExplanationNorm(randomContext, randomExplanation)));
        }
        return coveringRules;
	}
}
